...
package ESP32_C6_SVD.DS is
   ...
   --  memory that stores Y
   type Y_MEM_Registers is array (0 .. 511) of HAL.UInt8;

   --  memory that stores M

   --  memory that stores M
   type M_MEM_Registers is array (0 .. 511) of HAL.UInt8;

   --  memory that stores Rb

   --  memory that stores Rb
   type RB_MEM_Registers is array (0 .. 511) of HAL.UInt8;

   --  memory that stores BOX

   --  memory that stores BOX
   type BOX_MEM_Registers is array (0 .. 47) of HAL.UInt8;

   --  memory that stores IV

   --  memory that stores IV
   type IV_MEM_Registers is array (0 .. 15) of HAL.UInt8;

   --  memory that stores X

   --  memory that stores X
   type X_MEM_Registers is array (0 .. 511) of HAL.UInt8;

   --  memory that stores Z

   --  memory that stores Z
   type Z_MEM_Registers is array (0 .. 511) of HAL.UInt8;
